...
function renderMolecule2D(mol) {
  ...
  var drawBridgeManager = is3D
    ? Kekule.Render.DrawBridge3DMananger
  ...
  var rendererClass;
  if (is3D) rendererClass = Kekule.Render.get3DRendererClass(mol);
  else rendererClass = Kekule.Render.get2DRendererClass(mol);
  ...
  var options = Kekule.Render.RenderOptionUtils.convertConfigsToPlainHash(
    configObj
  );
  console.log(options);
